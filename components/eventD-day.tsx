...
const DDay = (props: any) => {
  ...
  const btDay =
    btMs / (1000 * 60 * 60 * 24) === 0 ? "Day" : btMs / (1000 * 60 * 60 * 24);

  setInterval(() => setDay(parseInt(new Date().getDate().toString())), 1000);

  return (
    <div className='flex flex-col items-center justify-center'>
      <h1 className={`${props.main ? "text-3xl" : "text-xl"}`}>
        {props.event}
      </h1>
      <h1
        className={
          "text-ms"
        }>{`${props.year}.${props.month}.${props.date}`}</h1>
      <h1 className={`${props.main ? "text-4xl" : "text-2xl"} text-green-300`}>
        {`D - ${btDay}`}
      </h1>
    </div>
  );
