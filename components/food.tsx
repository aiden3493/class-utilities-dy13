...
const Meal = () => {
  ...
  const Month =
    new Date().getMonth() < 10
      ? "0" + (parseInt(new Date().getMonth().toString()) + 1).toString()
      : (parseInt(new Date().getMonth().toString()) + 1).toString();
  ...
  async function fetchAPI(year: any, month: any, day: any) {
    const response = await fetch(
      `https://open.neis.go.kr/hub/mealServiceDietInfo?KEY=2d9929f041b342ba8e6701c69cd5ff24&Type=json&pIndex=1&pSize=10&ATPT_OFCDC_SC_CODE=J10&SD_SCHUL_CODE=7621058&MLSV_YMD=${year}${month}${day}`
    );
  ...
  const onNextClick = async (e: any) => {
    e.preventDefault();
    if (day < 31) {
      await fetchAPI(fullYear, Month, day + 1).then((value) =>
        setMeal(formatMeal(value))
      );
      await setDay((prev) => prev + 1);
    } else {
      await fetchAPI(fullYear, Month, 1).then((value) =>
        setMeal(formatMeal(value))
      );
      await setDay(1);
    }
  ...
  const onBeforeClick = async (e: any) => {
    ...
    if (day <= 31) {
      if (day - 1 === 0) {
        await fetchAPI(fullYear, Month, 31).then((value) =>
          setMeal(formatMeal(value))
        );
        await setDay(31);
      } else {
  ...
  return (
    <div className=" w-[22.5rem] h-[22.5rem] rounded-[35px] border-2 border-solid p-[37px] shadow-xl">
      <div className="text-center text-[40px] font-bold font-sans">
        <span className="text-2xl text-center font-medium">school </span>
        Lunch
      </div>
      ...
      <div className="relative w-full flex justify-center ">
        ...
        >
          <h1 className="mr-[3px] mt-[1px] text-center flex  content-center justify-center">
            ◁
          </h1>
        </div>
        ...
        >
          <h1 className="ml-[3px] mt-[1px] text-center flex content-center justify-center">
            ▷
          </h1>
        </div>
      </div>

      <div className="mt-3 w-full h-[220px] border-solid border-green-300 border-2 rounded-3xl select-text p-5 flex flex-col items-center font-bold">
